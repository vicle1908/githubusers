...
data class UserUi(val id: Int, val username: String, val avatarUrl: String, val htmlUrl: String? = null)
...
interface FeatureUsersRepository {
    fun getUsersPaged(): Flow<PagingData<UserUi>>
...
sealed interface UsersIntent : ViewIntent {
    ...
    data class SelectUser(val username: String) : UsersIntent
}

// State for list screen (browse only)
data class UsersState(val isRefreshing: Boolean = false, val error: String? = null) : ViewState
...
sealed interface UserDetailState : com.example.githubusers.core.mvi.contracts.ViewState {
    ...
    data class Loaded(val detail: UserDetailUi) : UserDetailState
