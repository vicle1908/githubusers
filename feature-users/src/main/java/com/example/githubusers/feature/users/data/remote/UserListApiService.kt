...
class UserListApiService @Inject constructor(
    ...
    private val usersEndpoint: String = "${baseUrl.trimEnd('/')}/users"
     ...
     */
    suspend fun getUsers(since: Int? = null, perPage: Int = 30): Result<List<UserSummaryDto>> = try {
        Timber.tag(TAG).d("Fetching users: since=$since, perPage=$perPage")
        Timber.tag(TAG).d("Making request to: $usersEndpoint")
