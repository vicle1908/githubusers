...
class UserRepositoryImpl @Inject constructor(
    ...
    override fun getUsersPaged(): Flow<PagingData<User>> = userListRepository.getUsersPaged().map { pagingData ->
    ...
    override suspend fun getUserDetail(username: String): Result<UserDetail?> =
        userDetailRepository.getUserDetail(username).map { detailEntity ->
            UserDetail(
                id = detailEntity.id,
