...
Claude integrates with Byterover for shared long-term memory management in AI workflows. Use Byterover tools for knowledge storage and retrieval:

- **Memory Layer**: Byterover serves as the persistent memory system for Claude's research and planning outputs.
- **Workflow Integration**: After Claude analysis, store findings using `mcp_byterover-mcp_byterover-store-knowledge` and retrieve context with `mcp_byterover-mcp_byterover-retrieve-knowledge` for continuity.
- **Setup**: Install ByteRover extension in your IDE, create access token at https://byterover.dev, and configure MCP to connect. For details, see [byterover-rules.md](./byterover-rules.md) Integration with IDEs and Setup section.
- **Benefits**: Ensures Claude's insights are preserved in the shared memory layer, enabling multi-session awareness and consistent decision-making across AI assistants.
