...
1. App is launched via `adb shell am start`
1. Another app sends an intent with the deep link
...
1. **Define Deep Links**: Create URI patterns for each navigation destination
1. **Implement Handlers**: Create `DeepLinkHandler` for each module
1. **Update Navigation Code**: Replace direct navigation with `moduleNavigator.navigateToModule()`
1. **Test**: Verify all navigation paths work correctly
