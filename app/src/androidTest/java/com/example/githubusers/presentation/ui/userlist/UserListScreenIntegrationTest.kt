...
class UserListScreenIntegrationTest {
    ...
    @Test
    fun searchBar_integration_searchForDefunkt() =
        runTest {
            ...
            assert(
                hasResults ||
                    composeTestRule.onAllNodes(hasText("No results found", substring = true)).fetchSemanticsNodes().isNotEmpty(),
            ) { "Should either show results or no results message" }
        }

    @Test
    fun searchBar_clearButton_clearsSearchAndShowsAllUsers() =
        runTest {
            composeTestRule.setContent {
                GithubUsersTheme {
                    val viewModel: UserListViewModel =
    ...
    @Test
    fun searchState_persistsWhenCollapsing() =
        runTest {
            composeTestRule.setContent {
                GithubUsersTheme {
                    val viewModel: UserListViewModel =
